...
## Authentication
- Endpoints for adding (`POST /discounts`), retrieving a specific discount (`GET /discounts/<int:discount_id>`), updating (`PUT /discounts/<int:discount_id>`), deleting (`DELETE /discounts/<int:discount_id>`), and retrieving all discounts (`GET /discounts`) require admin privileges, enforced by the `@admin_required` decorator.
...
- **Optional Fields**:
  - `max_uses` (integer): The maximum number of times the discount can be used (null for unlimited).
  - `expires_at` (string, ISO 8601 format): The expiration date and time of the discount.
...
- **Success Response** (HTTP 201):
  ...
  ```
- **Error Responses**:
  - **HTTP 400**: Missing required fields (`code` or `discount_percent`), invalid `discount_percent` (not between 0 and 100), or invalid `max_uses` (negative value).
    ```json
    {
...
### Method: `GET`
### Description
Retrieves a valid discount by its code (must be active and not expired or exceeded max uses). This endpoint is accessible to authenticated users.
...
- **Request Body** (Content-Type: `application/json`):
  - **Optional Fields**:
    ...
    - `max_uses` (integer): The updated maximum number of uses (null for unlimited).
    - `expires_at` (string, ISO 8601 format): The updated expiration date and time.
...
- **Success Response** (HTTP 200):
  ...
  ```
- **Error Responses**:
  - **HTTP 400**: Invalid `discount_percent` (not between 0 and 100), invalid `max_uses` (negative value), or failure to update the discount.
    ```json
    {
      "error": "Discount percent must be between 0 and 100"
    }
...
## Notes
- All endpoints interact with the database through the `DiscountManager` class.
...
- The `expires_at` field in requests and responses is in ISO 8601 format (e.g., `2025-08-31T23:59:59Z`).
- Error responses include a descriptive `error` field to assist clients in troubleshooting.
- The `GET /discounts/code/<string:code>` and `GET /discounts/valid/<string:code>` endpoints are accessible to authenticated users, while all other endpoints are restricted to admins.
- The `GET /discounts/valid/<string:code>` endpoint returns only discounts that are active (`is_active = 1`), not expired (current time is before `expires_at`), and have not exceeded their `max_uses` limit (if applicable).